...
|--------|-------------|----------|---------|
| `X-Tenant` | Unique tenant identifier for Jax Karwash | Always | `jax-karwash-001` |
...
- Jax Karwash will be provisioned with a unique `X-Tenant` identifier and `X-Tenant-API-Key`
  ...
  "vehicleData": [
    {
      ...
      "subscription": {
        ...
        "status": "active",
        "plan_name": "Jax Premium Unlimited Wash",
        "plan_type": "unlimited",
        "price": 29.99,
        "currency": "USD",
...
**Note**: The `vif_id` and `color_id` are typically obtained from a separate vehicle search/lookup API that's not covered in this integration spec. For Jax Karwash implementation, these can be simple lookup tables or IDs that reference your vehicle database.
