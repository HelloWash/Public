...
**Data hierarchy note:** ~~We do not assume how Jax models accounts, members, plans, memberships, or vehicles.~~ **Users have many Accounts; each Account has one Vehicle and one Subscription.** *Marvin's context* Terminology (e.g. "subscription", "plan", "membership") is used generically. Map our terms to your entities and IDs as appropriate. Where an endpoint could be "by account", "by member", or "by vehicle", we describe it generically so you can align with your structure.
...
- [Priority Endpoints (Support First)](#priority-endpoints-support-first)
  ...
  - [6. Get Retention Offer](#6-get-retention-offer)
  - [7. Apply Retention Offer](#7-apply-retention-offer)
  - [8. Cancel Plan / Apply Cancellation](#8-cancel-plan--apply-cancellation)
  - [Offer request and response](#offer-request-and-response)
- [Retention offer types](#retention-offer-types)
...
- [Membership operations (optional)](#membership-operations-optional)
...
**Value summary** means wash history/savings we play back before presenting an offer; **retention offer** means the offer we present and may apply. These can be one combined endpoint or value summary nested inside the retention-offer response—endpoint names and shapes are flexible. Each operation (value summary, retention offer) has a single responsibility and a clear data model; how they are delivered (one combined endpoint, separate endpoints, or value summary nested in the offer response) is flexible.
...
**When there are multiple results:** We use alternative lookups (email, license plate) or clarifying questions until we **isolate the single plan** to cancel. Only then do we fetch value summary and retention offer.
...
For Jax, multiplicity is at the account level: each account has one vehicle and one subscription; we disambiguate to a single account when needed. *Marvin's context*
...
The first step is to get customer + plan + vehicle. That may be **one call** (lookup-by-phone response includes customer, plan, and vehicle) or **two calls** (lookup-by-phone then get-info when plan/vehicle are not in the lookup response). Value and offer come in a later combined call.
...
| **Lower priority (useful later)** | Deeper data and lifecycle operations | Get vehicle/data by identifier, get plan by key, create plan/membership (if needed) — see **Lower Priority Endpoints** section at end of document. |

*Value summary and retention offer may be: separate endpoints (single responsibility) or one combined endpoint.*

**Operational Flows** below show the step-by-step call sequences; **Priority Endpoints** then detail each API; offer shape variations and reference follow.
...
<del>**Authentication:** You can choose your own authentication mechanism (e.g. API key, Bearer token, or OAuth2). The following is for illustration only; use whatever fits your setup. **Header:** `Api-Key` (or `Authorization: Bearer <token>` for Bearer, or OAuth2 access token in `Authorization` if you use OAuth2).</del>

**Authentication:** **API key** (e.g. `Api-Key` header); Jax will provide the key via secure channel. **IP whitelisting:** we will provide the IP address(es) our system uses to call the API. *Marvin's context*

- **Content-Type:** `application/json` for all POST requests.

<del>*This integration is single-tenant; tenant-scoping headers (e.g. `X-Tenant-Id`) are not required.*</del> Single-tenant; no tenant-scoping headers. *Marvin's context*

---

## Operational Flows (Priority)

**Note:** The flows below illustrate the general approach with validation, confirmation, and error handling. Specific prompts, confirmation wording, and error recovery strategies will be refined through discussion with tech teams and Meddy. Request/response details for each API step are in **Priority Endpoints (Support First)** below.

### Flow 1: Cancel by Phone

**Diagram 1a – Call start (phone):** Incoming call; system looks up by known phone, then greets with context. If not found, see Lookup failure and fallback (Overview section). Customer + plan + vehicle come from the first step (either one lookup response that includes them, or lookup then get-info). If the lookup response already includes plan/vehicle, the get-info call may be omitted. Value summary and retention offer are fetched later (combined or separate).
...
sequenceDiagram
    ...
    alt Offer available
        API-->>System: retention_offer_id, description, duration_months, discount_percent, expires_at
        ...
        Customer->>System: "Yes"
        System->>API: POST /api/retention/apply-offer
        API-->>System: success, applied_offer details
        System->>Customer: "Offer applied successfully."
    else No offer
        API-->>System: retention_offer_id null
        System->>Customer: "No offers available at this time."
    end
```
...
The following endpoints are what we expect to need for the core voice AI flows: identify the customer, understand their plan/membership, play back value summary when cancelling, perform cancellation, and handle retention offers. The endpoint paths and names below (e.g. `POST /api/retention/get-offer`, `POST /api/plans/value-summary`) are **examples** and can be combined or renamed to match how you group data (e.g. value summary + retention combined) as described in the Integration approach and Endpoint grouping options above.
...
**Retention offers in context:** In the cancel flow we **get** an offer for the plan we're cancelling, **present** it, then either **apply** it or confirm cancel. Get-offer and apply-offer use **one request/response shape** for all offer types; type-specific details use optional fields so the voice AI can state terms correctly. This doc details the **multi-month** retention offer (discount over X months for the customer's current tier; discount can be by amount or percent depending on plan). Other offer types are summarized briefly.

### 6. Get Retention Offer

**Priority:** Yes

**Purpose:** Return the retention offer (if any) for the given plan. The response must include a stable offer ID for use in apply-offer. How you link the offer to your data (plan, account, membership) is up to you. This can be a **separate endpoint or combined with value summary**; when combined, we use that single response for playback and for the offer. If there's no offer (e.g. no value to show), return the offer ID as null—we skip the offer and go to cancellation confirmation.

**Endpoint:** `POST /api/retention/get-offer`

*Path is flexible (e.g. `/api/retention/get-offer`, `/api/offers/get`, or equivalent); we will align to your conventions.*
...
*`retention_offer_id` must be unique and usable in the apply endpoint. Optional `offer_type` (e.g. `multi_month`) helps the voice AI phrase the terms correctly. You can also include IDs that tie to your hierarchy (e.g. plan_id, account_id) if needed.*
...
*The same request/response shape is used for all offer types; see **Offer request and response** below.*
...
**Purpose:** Apply or redeem the offer. Call with the **offer ID** from get-offer (or from the combined value-summary + retention-offer response) and the identifiers you need (e.g. `customer_id`, `plan_id`). We can pass back any IDs you returned in get-offer or get-info.

**Endpoint:** `POST /api/retention/apply-offer`

*Path is flexible (e.g. `/api/retention/apply-offer`, `/api/offers/apply`, or equivalent); we will align to your conventions.*
...
*If you need additional IDs (e.g. account_id, membership_id) to apply the offer, add those fields; we can pass back any IDs you returned in get-offer or get-info.*
...
*Same request/response shape for every offer type; see **Offer request and response** below.*
...
Get-offer and apply-offer use **one request and one response shape** for all offer types. Type-specific details are expressed with optional fields (e.g. `offer_type`, `duration_months`, `free_washes_count`) so the voice AI can phrase the terms correctly. The offer identifier may be named `offer_id` or `retention_offer_id`; both refer to the same value.

**Get-offer**

- **Request:** `customer_id`, `plan_id`, and any other IDs you need (e.g. `account_id`, `membership_id`).
...
Multi-month uses `duration_months` and discount fields; one-time omits `duration_months`; N free washes uses `free_washes_count`; other uses `description` and optional extra fields.

**Apply-offer**

- **Request:** `retention_offer_id` (or `offer_id`), `customer_id`, optional `plan_id` (and any other IDs required to apply).
...
- **Response:** `success`, `message`; optional `retention_offer_id`, `applied_at`, `discount_code`. Same shape for every offer type.
...
Sections 6 and 7 use the multi-month example; the same contract applies to all offer types below.

---

## Retention offer types

The following types use the get-offer and apply-offer request/response shape above. This doc details **multi-month** only; you don't need to support all types at once. The offer identifier may be `offer_id` or `retention_offer_id` (same value). The examples in sections 6 and 7 and in **Offer request and response** use one consistent multi-month data set.

---

**1. Multi-month retention offer** *(primary example in this doc)*

- **What it is:** The primary example in this doc: a discount over X months for the customer's **current tier**. Depending on plan, the discount is defined as an **amount** or a **percent** (e.g. 50% off for 3 months, or $X off per month for 6 months).
- **Get-offer returns:** e.g. `retention_offer_id`, `description`, `duration_months`, `discount_percent` and/or `discount_amount` (and optionally `discount_currency`), `expires_at`; optional `plan_id` for consistency.
- **Apply-offer does:** Accepts `retention_offer_id` and identifiers (`customer_id`, `plan_id`); backend applies the reduced rate to that plan for that duration.
- **Voice AI:** States the terms to the customer; if they accept, calls apply-offer.

---

**2. One-time or fixed discount**

- **What it is:** A single-use discount (e.g. “50% off your next wash” or “$5 off next visit”).
- **Get-offer returns:** e.g. `retention_offer_id`, `description`, `expires_at` (no `duration_months`).
- **Apply-offer does:** Redeems the offer once; backend marks it used and applies the discount to the next eligible wash or bill.
- **Voice AI:** States the terms; if they accept, calls apply-offer.

---

**3. Other offer shapes**

Other forms (e.g. N free washes, percent off for a set number of washes, tiered discounts) use the same request/response shape with optional fields (e.g. `free_washes_count`, `offer_type`) and a clear `description` so the voice AI can state the terms correctly.
...
|------|-----------|-----------------------------------|-------------------|---------------------|
| 1 | Lookup by phone | `POST /api/customers/lookup-by-phone` or `/api/members/lookup-by-phone`, `/api/accounts/lookup-by-phone` | `{"phone": "5551234567"}` | `{"customer_id": "12345", "name": "John Doe", "email": "...", "phone_number": "..."}` — may include plans/vehicles or list for disambiguation (Jax: one vehicle + one subscription per account, or list of accounts for disambiguation) *Marvin's context* |
...
Offer request/response for get-offer and apply-offer follow **Offer request and response** (see above).

For get-by-identifier, get-plan-by-key, and create-plan operations, see **Lower Priority Endpoints** at the end of this document.
...
4. **ID consistency:** Same identifier from lookup (and get-info when used) is used for value-summary, cancel, and retention; the offer ID (`offer_id` or `retention_offer_id`) from get-offer is used in apply-offer.
5. **Hierarchy:** If you use account/member/vehicle distinctions, test with the IDs and field names that match your model.
6. If the back end supports both separate and combined value-summary/retention-offer options, test both.
7. **Auth and IP:** API key as agreed; provide Jax with outbound IP(s) for whitelisting. *Marvin's context*
8. **Response shape:** vehicle/subscription as single object `{ "vehicle": {}, "subscription": {} }` per account. *Marvin's context*
...
Optional endpoints for lifecycle operations; consider when the back end and voice flow support them. No need to support all at once.
...
- **Downgrade:** e.g. `POST /api/plans/downgrade` or `POST /api/membership/downgrade` — plan_id, target_plan_id (or equivalent). May require listing eligible plans.
- **Upgrade:** e.g. `POST /api/plans/upgrade` or `POST /api/membership/upgrade` — plan_id, target_plan_id (or equivalent). May require listing eligible plans.
